...
### 8.8 Monitoreo de drift (Evidently, RNF-4)

Octavo y último paso de la secuencia.
Depende de tener un flujo de scoring real corriendo (`/score`, paso 2) sobre el cual medir drift.

**El problema de un dataset estático.**
Home Credit Default Risk es un dataset fijo de Kaggle: no hay solicitudes llegando en el tiempo, así que "drift" no puede medirse contra tráfico productivo real como en un sistema en producción.
La decisión de diseño, planteada explícitamente al usuario antes de escribir código, era entre usar el holdout real de Kaggle (`application_test`) tal cual como población "actual", aceptando el riesgo de que el resultado diera poco o nada de drift, o perturbar sintéticamente una columna para garantizar que la alerta se viera disparándose en la demo.
Se optó por los datos reales sin perturbar: `application_test` (`IS_TRAIN==0` en `features.parquet`) son solicitudes reales que Kaggle reserva sin `TARGET` para su competencia, nunca usadas para entrenar el modelo, genuinamente disjuntas de `application_train` (`IS_TRAIN==1`), que funciona como referencia.

**Logs de predicción (RNF-4).**
`GET /score/{sk_id_curr}` en `app/api.py` loguea cada predicción real con `credixai.monitoring.logging.append_log_entry` a `models/monitoring/prediction_log.jsonl` (append-only, timestamp + `sk_id_curr` + probabilidad + decisión); el path del log se inyecta vía `get_prediction_log_path()` con `Depends`, mismo patrón de DI que el resto del módulo, así los tests escriben a un archivo temporal en vez de al log real.
Solo se loguean predicciones exitosas: un `SK_ID_CURR` inexistente (404) no genera entrada, porque no hubo predicción real que registrar.

**TDD.**
`format_log_entry` (arma la entrada de log a partir de un `ScoreResult`, recibiendo el timestamp como parámetro en vez de generarlo, para no depender del reloj real en el test) y `alert_columns` (filtra columnas con PSI por encima del umbral) son funciones puras en `credixai/monitoring/`, testeadas sin IO ni Evidently real.
`append_log_entry` sí tiene IO real (crea el directorio padre si no existe, escribe en modo append) y se testeó contra archivos reales en `tmp_path`, no mockeado, mismo criterio que `vector_store.py` en el paso 5 (validar el wiring real de IO en vez de una simulación).
El wiring en `/score` se testeó con `dependency_overrides` apuntando a `tmp_path`, verificando que cada llamada exitosa agrega una línea y que un 404 no agrega ninguna.
Al escribir el primer test de `/score` con logging se detectó que el test ya existente `tests/test_api_http.py` no sobreescribía `get_prediction_log_path` y por lo tanto escribía de verdad en `models/monitoring/prediction_log.jsonl` durante la suite rápida; se corrigió agregando el mismo override, para que ningún test de la suite rápida tenga efectos secundarios sobre el filesystem real del proyecto.
El reporte de Evidently en sí (`scripts/09_drift_report.py`) no tiene test unitario, mismo criterio que `07_rag_eval.py` y `08_langfuse_judge_validation.py`: se verifica manualmente contra el sistema real.

**Reporte de drift y resultado real.**
`scripts/09_drift_report.py` batch-scorea las ~48.700 filas de `application_test` con el modelo ya entrenado (mismo bundle que sirve `/score`), loguea cada predicción con el mismo mecanismo que la API, y corre `evidently.report.Report` con `DataDriftPreset(stattest="psi", stattest_threshold=0.2)` comparando la distribución de las 301 columnas de features entre referencia y actual.
Guarda el reporte completo en `models/monitoring/drift_report.html` y resume en stdout las columnas que superan el umbral de alerta de RNF-4 (PSI > 0.2) vía `alert_columns`.

Resultado real, sin forzar: 6 de 301 columnas superan el umbral.
La más marcada es `bb_no_record` (PSI≈1.55, el flag de "sin historial en `bureau_balance`" creado en la Tarea 2): la proporción de solicitudes sin historial de `bureau_balance` difiere sustancialmente entre `application_train` y `application_test`, consistente con la cobertura desigual de esa tabla por SK_ID_CURR ya documentada en la sección de feature engineering.
